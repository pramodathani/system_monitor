...
UBI trims streams with approximate `MAXLEN`: 1,000,000 for broker quote streams and for `unified:quotes:stream`, 20,000 for broker update streams, and 50,000 for the unified update streams. The quote caps were 100,000 and 200,000 until UBI raised them on 2026-09-19. On 2026-09-15 `zerodha:quotes:stream` sat at exactly its cap of 100,000 entries, so trimming happens continuously. A consumer group that falls behind by more than the cap loses the oldest unread entries for good, and for a `persist` group that means ticks or order updates missing from TimescaleDB. Half the cap is the failure line because trimming is approximate and a lag growing that fast will cross the cap soon.
