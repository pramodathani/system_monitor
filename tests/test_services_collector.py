...
class TestServicesCollector:
    ...
    def test_collect_skips_timers_and_scheduled_units(self):
        ...
        systemd_client.properties['zerodha@quotes.service'] = self._running()
        results = self._by_id(collector.collect())
        assert set(results) == {
            'services:zerodha@quotes.service',
            'services:zerodha@orders.service',
    ...
    def test_collect_running_is_ok_with_uptime(self):
        ...
        collector, systemd_client, _clock = self._collector()
        systemd_client.properties['zerodha@quotes.service'] = self._running()
        result = self._by_id(collector.collect())['services:zerodha@quotes.service']
    ...
    def test_collect_missing_unit_is_failure(self):
        ...
        result = self._by_id(collector.collect())['services:zerodha@orders.service']
    ...
    def test_collect_auto_restart_depends_on_kind(self):
        ...
        systemd_client.properties['zerodha@quotes.service'] = waiting
        systemd_client.properties['zerodha-historical-prices.service'] = waiting
        results = self._by_id(collector.collect())
        assert results['services:zerodha@quotes.service'].status == CheckStatus.WARNING
    ...
    def test_collect_failed_unit_is_failure(self):
        ...
        """
        collector, systemd_client, _clock = self._collector()
        systemd_client.properties['zerodha@quotes.service'] = {
            'LoadState': 'loaded',
            'ActiveState': 'failed',
            'SubState': 'failed',
            'Result': 'exit-code',
    ...
    def test_collect_recent_restarts_warn_then_clear(self):
        ...
        systemd_client.properties['zerodha@quotes.service'] = self._running(restarts=2)
        assert self._by_id(collector.collect())['services:zerodha@quotes.service'].status == CheckStatus.OK

        clock.advance(60)
        systemd_client.properties['zerodha@quotes.service'] = self._running(restarts=3)
        result = self._by_id(collector.collect())['services:zerodha@quotes.service']
